...
@interface SCIRenderableSeriesBase : NSObject <SCIRenderableSeriesProtocol, SCIThemeableProtocol> {
    // Point series from last main render loop. The series is not changed while animation loop is running.
    @protected id<SCIPointSeriesProtocol> _previousPointSeries;
    
    // Animations controller contains all nimation of the renderable series.
    @protected SCIAnimationsController * _animationsController;
    
    // Data series observer of changing data.
    @protected SCIActionBlock _dataSeriesObserver;
}
